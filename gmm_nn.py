...
sigma_results = pd.DataFrame(columns=['mse','delta_A','sigma','seed','mmse','delta','eps_c','d','n','h', 'val_n','val_mse', 'eps_c_tilde'])
...
for sigma in sigma_range:
    ...
    nn = gdNN(loss='square',device='cuda').fit(X_big, y_big,lr=1e-2, scheduler='cosine', epochs=2000, opt='AdamW', n_hidden = h)
    ...
    for n in n_range:
        for seed in tqdm(range(30),leave=False):
            ...
            LR = gdNN(loss='square',device='cuda').fit(X, y,lr=1e-2, scheduler='cosine',epochs=2000,opt='AdamW',n_hidden = h)
            if first:
                file_path = os.path.join(args.model_path,f'gmm_{args.dim}_{args.hidden}_{val_n}.pth.tar.gz')
            ...
            sigma_results.loc[len(sigma_results)] = ( lin_mse,delta_a, sigma, seed, mg.mmse_estimate(1e6),delta,eps_c,d,n,h,val_n, val_mse, eps_c_tilde)
sigma_results.to_csv(os.path.join(args.results_path, f'gmm_{args.sigma}_{args.n}_{args.delta}_{args.dim}_{args.hidden}_{val_n}.csv'),mode='a')
