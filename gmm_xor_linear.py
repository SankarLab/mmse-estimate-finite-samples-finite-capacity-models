...
sigma_results = pd.DataFrame(columns=['mse','delta_A','sigma','seed','mmse','delta','eps_c','n_modes','n', 'val_n','val_mse', 'eps_c_tilde'])
prefix = 'gmm_xor_linear'
...
for sigma in sigma_range:
    ...
    for n in n_range:
        for seed in tqdm(range(30),leave=False):
            ...
            if first:
                file_path = os.path.join(args.model_path,f'{prefix}_{args.n_modes}_{val_n}.pth.tar.gz')
            ...
            sigma_results.loc[len(sigma_results)] = ( lin_mse,delta_a, sigma, seed, mg.mmse_estimate(1e6),delta,eps_c,n_modes,n,val_n, val_mse, eps_c_tilde)
sigma_results.to_csv(os.path.join(args.results_path, f'{prefix}_{args.sigma}_{args.n}_{args.delta}_{args.n_modes}_{val_n}.csv'),mode='a')
